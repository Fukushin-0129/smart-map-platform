...
const elements = {
  ...
  flyerSyncStatus: document.querySelector('#flyerSyncStatus'),
  migrateFlyersButton: document.querySelector('#migrateFlyersButton'),
...
async function initializeFlyerStorage() {
  ...
  if (!isSupabaseConfigured()) {
    updateFlyerSyncStatus('オフラインデータを使用しています');
    showToast('オフラインデータを使用しています');
    return;
  }
...
function mergeFlyerSources(localFlyers, remoteFlyers) {
  ...
  [...localFlyers, ...remoteFlyers].forEach((apt) => {
...
function isNewerFlyer(a, b) {
  return Date.parse(a.updatedAt || a.createdAt || 0) >= Date.parse(b.updatedAt || b.createdAt || 0);
}

function updateFlyerSyncStatus(message) {
  if (elements.flyerSyncStatus) elements.flyerSyncStatus.textContent = message;
}

function updateMigrationButton(visible) {
  if (elements.migrateFlyersButton) elements.migrateFlyersButton.hidden = !visible;
}

function queueFlyerSync(items) {
  const merged = new Map(loadFlyerSyncQueue().map((apt) => [apt.id, apt]));
  items.forEach((apt) => merged.set(apt.id, apt));
  saveFlyerSyncQueue(Array.from(merged.values()));
  updateFlyerSyncStatus(`同期待ち ${merged.size}件`);
}

async function flushFlyerSyncQueue() {
  const queue = loadFlyerSyncQueue();
  if (!queue.length || !isSupabaseConfigured()) return;
  const result = await saveFlyerPlacesToSupabase(queue);
  if (result.ok) {
    saveFlyerSyncQueue([]);
    updateFlyerSyncStatus('同期待ちデータを送信しました');
    showToast('同期待ちデータを送信しました');
  } else {
    updateFlyerSyncStatus(`同期待ち ${queue.length}件`);
  }
}
...
function persistFlyerApartments(itemsToSync = flyerApartments) {
  ...
  saveFlyerPlacesToSupabase(itemsToSync).then((result) => {
    if (result.ok) {
      updateFlyerSyncStatus('同期済み');
      showToast('共有データへ保存しました');
      flushFlyerSyncQueue();
    } else {
...
function bindEvents() {
  ...
  if (elements.migrateFlyersButton) elements.migrateFlyersButton.addEventListener('click', migrateLocalFlyersToSupabase);
  window.addEventListener('online', flushFlyerSyncQueue);
...
function openFlyerRegistrationPanel(mode = 'menu') {
  ...
  if (mode === 'search' && !autocompleteService) {
    elements.mapStatus.textContent = '検索機能を利用できません。地図から位置を指定してください';
  }
  renderFlyerRegistrationPanel();
  elements.flyerRegistrationPanel.hidden = false;
  setTimeout(() => elements.flyerRegistrationPanel.querySelector('#flyerPlaceSearchInput')?.focus(), 0);
}
...
function renderFlyerRegistrationPanel() {
  ...
  elements.flyerRegistrationPanel.innerHTML = `
    <div class="flyer-registration-header"><div><p class="drawer-eyebrow">チラシ配布先登録</p><h2>検索から新規登録</h2></div><button type="button" data-close-flyer-registration>×</button></div>
    ...
    ${selected ? renderFlyerSelectedPlace(selected, assigneeOptions) : ''}`;
...
function renderFlyerSelectedPlace(place, assigneeOptions) {
  ...
  return `<form id="flyerRegistrationForm" class="flyer-registration-form">
    ...
    <label>初期ステータス<select name="status">${FLYER_STATUSES.map((s) => `<option value="${escapeHtml(s)}" ${s === '未配布' ? 'selected' : ''}>${escapeHtml(s)}</option>`).join('')}</select></label>
...
function setSelectedFlyerPlace(place) {
  ...
  if (map) { map.panTo({ lat: place.lat, lng: place.lng }); map.setZoom(Math.max(map.getZoom() || 16, 17)); }
  tempFlyerMarker?.setMap(null);
...
function startCurrentLocationFlyerRegistration() {
  ...
  if (!navigator.geolocation) { showToast('このブラウザは現在地取得に対応していません'); return; }
  navigator.geolocation.getCurrentPosition((pos) => {
    if (!window.google?.maps) { showToast('検索機能を利用できません。地図から位置を指定してください'); return; }
    selectFlyerMapLocation(new google.maps.LatLng(pos.coords.latitude, pos.coords.longitude));
...
function findFlyerDuplicates(place) {
  ...
  return flyerApartments.filter((apt) => {
    ...
    return nameKey && addressKey && normalizeDuplicateText(apt.name) === nameKey && normalizeDuplicateText(apt.address) === addressKey;
  });
}
function normalizeDuplicateText(value) { return String(value || '').toLowerCase().replace(/[\s　、，,.-]/g, ''); }

function registerSelectedFlyerPlace(event) {
  event.preventDefault();
  if (!selectedFlyerPlace) return;
  if (flyerDuplicateCandidates.length && !allowDuplicateFlyerRegistration) { showToast('重複候補を確認してください'); renderFlyerRegistrationPanel(); return; }
  const data = new FormData(event.currentTarget);
  const now = new Date().toISOString();
  const apt = { id: crypto.randomUUID(), layerId: FLYER_LAYER.id, layerName: FLYER_LAYER.name, name: data.get('name').trim(), address: data.get('address').trim(), lat: Number(selectedFlyerPlace.lat), lng: Number(selectedFlyerPlace.lng), status: data.get('status') || '未配布', assignee: data.get('assignee') || '', memo: data.get('memo').trim(), placeId: selectedFlyerPlace.placeId || '', createdAt: now, updatedAt: now, photos: [] };
...
function rowsToFlyerApartments(rows) {
  ...
  const imported = rows.slice(headerIndex + 1).map((row) => {
    ...
    return {
      ...
      updatedAt: saved?.updatedAt || importedAt,
    };
  }).filter(Boolean);
  return imported;
}
...
function updateFlyer(id, patch, options = {}) {
  ...
  persistFlyerApartments(flyerApartments.filter((apt) => apt.id === id));
  renderFlyerList();
  renderFlyerRoutes();
  renderMarkers();
