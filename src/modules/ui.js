...
app.innerHTML = `
  ...
  <main class="layout">
    <aside class="panel controls" aria-label="店舗管理">
      <section class="legacy-sidebar" hidden>
        <h2>店舗登録</h2>
      ...
      <section>
        ...
        <label>配布状況で絞り込み<select id="statusFilter"></select></label>
      ...
      <section class="full-list-section" hidden>
        <div class="list-header">
          <h2>店舗一覧</h2>
    ...
    <section class="panel map-panel" aria-label="地図">
      ...
      <div id="boundsFlyerPanel" class="bounds-flyer-panel"></div>
      <div id="mobileFlyerDetail" class="mobile-flyer-detail" hidden></div>
...
const elements = {
  ...
  boundsFlyerPanel: document.querySelector('#boundsFlyerPanel'),
  mobileFlyerDetail: document.querySelector('#mobileFlyerDetail'),
...
export async function initializeApp() {
  ...
  try {
    ...
    map.addListener('idle', renderBoundsFlyerPanel);
    renderMarkers();
    fitMapToVisibleData();
...
function bindEvents() {
  ...
  elements.assigneeFilter.addEventListener('change', () => { flyerRoutes = []; renderFlyerRoutes(); renderFlyerList(); renderBoundsFlyerPanel(); renderMarkers(); fitMapToVisibleData(); });
  elements.statusFilter.addEventListener('change', () => { flyerRoutes = []; renderFlyerRoutes(); renderFlyerList(); renderBoundsFlyerPanel(); renderMarkers(); fitMapToVisibleData(); });
...
function renderMarkers() {
  ...
  flyerMarkers = filteredFlyerApartments().map((apt) => {
    ...
    marker.addListener('click', () => showFlyerDetail(apt, marker));
    return marker;
  });
}
...
function renderStatusFilter() {
  ...
  elements.statusFilter.innerHTML = '<option value="">すべて</option>' + FLYER_STATUSES.map((status) => `<option value="${escapeHtml(status)}" ${status === selected ? 'selected' : ''}>${escapeHtml(status)}</option>`).join('');
}
...
function rowsToFlyerApartments(rows) {
  ...
  return rows.slice(1).map((row) => {
    ...
    const latText = get('緯度', '緯度・経度', '緯度経度', '座標', 'lat', 'latitude');
    ...
    const assignee = normalizeFlyerAssignee(get('担当者', '担当'));
    return {
      id: crypto.randomUUID(), layerId: FLYER_LAYER.id, layerName: FLYER_LAYER.name, name: get('物件名', '建物名', 'マンション名', '名称') || '名称未設定',
      address: get('住所', '所在地'), area: get('エリア'), type: get('種別'), schoolDistrict: get('小学校区'), units: get('戸数'), unitStatus: get('戸数状態'),
      distributionDate, nextDistribution: get('次回配布'), memo: get('備考', 'メモ'), lat, lng, status, assignee, photos: [], createdAt: new Date().toISOString(),
    };
  }).filter(Boolean);
}
...
function filteredFlyerApartments() {
  ...
  return flyerApartments.filter((apt) => isLayerVisible(FLYER_LAYER.id) && (!assignee || apt.assignee === assignee) && (!status || apt.status === status) && (!keyword || [apt.name, apt.address, apt.area, apt.type, apt.schoolDistrict, apt.memo, apt.status, apt.assignee].some((v) => String(v || '').toLowerCase().includes(keyword))));
}
...
function updateFlyer(id, patch) {
  ...
  if (apt && !elements.mobileFlyerDetail.hidden) renderMobileFlyerDetail(apt);
  renderBoundsFlyerPanel();
}
...
function showFlyerDetail(apt, marker) {
  if (window.matchMedia('(max-width: 700px)').matches) {
    infoWindow?.close();
    renderMobileFlyerDetail(apt);
    return;
  }
  openFlyerInfo(apt, marker);
}

function flyerDetailHtml(apt, { mobile = false } = {}) {
  return `<div class="${mobile ? 'mobile-flyer-detail-card' : 'info-window flyer-info-window'}">
    ${mobile ? '<button type="button" class="mobile-detail-close" data-close-mobile-detail>×</button>' : ''}
    <strong>${escapeHtml(apt.name)}</strong>
    <p>${escapeHtml(apt.address || '住所なし')}</p>
    <p>配布状況: <strong>${escapeHtml(apt.status || '未配布')}</strong></p>
    <p>担当者: ${escapeHtml(apt.assignee || '未設定')}</p>
    <button type="button" class="primary flyer-complete-button" data-complete-flyer-detail="${escapeHtml(apt.id)}">配布済み</button>
  </div>`;
}

function bindFlyerDetailActions(root = document) {
  root.querySelectorAll('[data-complete-flyer-detail]').forEach((button) => {
    button.addEventListener('click', () => setFlyerStatusFromButton(button.dataset.completeFlyerDetail, '配布済み'));
  });
  root.querySelectorAll('[data-close-mobile-detail]').forEach((button) => {
    button.addEventListener('click', () => { elements.mobileFlyerDetail.hidden = true; });
  });
}

function openFlyerInfo(apt, marker) {
  infoWindow.setContent(flyerDetailHtml(apt));
  infoWindow.open({ anchor: marker, map });
  google.maps.event.addListenerOnce(infoWindow, 'domready', () => bindFlyerDetailActions(document));
}

function renderMobileFlyerDetail(apt) {
  elements.mobileFlyerDetail.innerHTML = flyerDetailHtml(apt, { mobile: true });
  elements.mobileFlyerDetail.hidden = false;
  bindFlyerDetailActions(elements.mobileFlyerDetail);
}

function boundsContainsApt(bounds, apt) {
  if (!bounds || !isValidCoordinate(apt.lat, apt.lng)) return false;
  return bounds.contains(new google.maps.LatLng(apt.lat, apt.lng));
}

function visibleFlyersInBounds() {
  if (!map || !window.google?.maps) return [];
  const bounds = map.getBounds();
  return filteredFlyerApartments().filter((apt) => boundsContainsApt(bounds, apt));
}

function renderBoundsFlyerPanel() {
  if (!elements.boundsFlyerPanel) return;
  const items = visibleFlyersInBounds();
  elements.boundsFlyerPanel.innerHTML = `
    <div class="bounds-panel-header"><strong>表示範囲の配布先</strong><span>${items.length}件</span></div>
    ${items.length ? `<div class="bounds-panel-list">${items.slice(0, 30).map((apt) => `<button type="button" data-focus-flyer="${escapeHtml(apt.id)}"><strong>${escapeHtml(apt.name)}</strong><small>${escapeHtml(apt.status)} / ${escapeHtml(apt.assignee || '未設定')}</small></button>`).join('')}</div>` : '<p class="empty">現在の地図範囲に配布先はありません。</p>'}`;
  elements.boundsFlyerPanel.querySelectorAll('[data-focus-flyer]').forEach((button) => button.addEventListener('click', () => focusFlyer(button.dataset.focusFlyer)));
}

function focusFlyer(id) { const apt = flyerApartments.find((item) => item.id === id); if (!apt || !map) return; map.setCenter({ lat: apt.lat, lng: apt.lng }); map.setZoom(16); const marker = flyerMarkers.find((item) => item.flyerId === id); if (marker) showFlyerDetail(apt, marker); }
